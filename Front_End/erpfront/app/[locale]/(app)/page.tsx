...
export default async function Home() {
	const [perms, email, t, tModules] = await Promise.all([
		fetchMyPermissions(),
		decodeAccessTokenEmail(),
		getTranslations("moduleLauncher"),
		getTranslations("modules"),
	]);

	const tiles: ModuleTile[] = MODULE_REGISTRY.map((entry) => ({
		...entry,
		label: tModules(`${entry.id}.label`),
		description: tModules(`${entry.id}.description`),
	...
	return (
		<div className="mx-auto w-full max-w-[1100px] flex-1 p-6">
			<div className="mb-4">
				...
				{email && <p className="text-[13px] text-muted">{t("signedInAs", { email })}</p>}
			</div>
